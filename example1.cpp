...
init()
{
    ...
    int w,h;
    loadBMP_custom(&pic0, &w, &h, "lid_01.bmp");
    
    glBindTexture( GL_TEXTURE_2D, textures[0] );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_BGR, GL_UNSIGNED_BYTE, pic0 );
    ...
    */
    unsigned char* pic0 = NULL;
    int w0, h0;
    loadBMP_custom(&pic0, &w0, &h0, "lid_01.bmp");
    ...
    int w1, h1;
    loadBMP_custom(&pic1, &w1, &h1, "PrimordialSoup.bmp");
    
    glBindTexture( GL_TEXTURE_2D, textures[1] );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, w1, h1, 0, GL_BGR, GL_UNSIGNED_BYTE, pic1 );
    glGenerateMipmap(GL_TEXTURE_2D);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    ...
    int w2, h2;
    loadBMP_custom(&pic2, &w2, &h2, "soup_can_bottom.bmp");
    ...
    pic0 = NULL;
    loadBMP_custom(&pic0, &w0, &h0, "food-tin-can-lid-18111983.bmp");
    //loadBMP_custom(&pic0, &w0, &h0, "Fry_Looking_Squint.bmp");
...
display( void )
{
    ...
    mat4 transform_bube = Translate( 0.0, 1.0, -1.0 ) * RotateY(180) * Scale(3.0, 3.0, 3.0);
    ...
    glActiveTexture( GL_TEXTURE0 );
    glBindTexture( GL_TEXTURE_2D, textures[3] );
    glDrawArrays( GL_TRIANGLES, 0, NumVerticesCylinder/4 );
    glBindTexture( GL_TEXTURE_2D, textures[1] );
    glDrawArrays( GL_TRIANGLES, NumVerticesCylinder/4, NumVerticesCylinder/2 );
    glBindTexture( GL_TEXTURE_2D, textures[2] );
    glDrawArrays( GL_TRIANGLES, NumVerticesCylinder*3/4, NumVerticesCylinder/4 );
